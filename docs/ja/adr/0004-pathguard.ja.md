...
- ファイルの床は向きで分ける: `UploadDenied`（Outbound。upload はマシンの外へ出るので、資格情報の名前はどこに
  ...
  途中のリンクをすべてたどる）、名指しされたパス（整えたもの）と置き場所の両方で判定する。
- **パスが存在するかどうかで答えを変えない。** 両方向とも 1 つの経路・1 つの順序にする: パスの置き場所を決める
  （`workdir.Where`。pathguard の形の最後で、リンクはすべて、壊れたリンクも行き先までたどる。存在するパスなら
  EvalSymlinks と同じ）→ 床 → 封じ込め → ディレクトリの検査 → 存在に依存する検査（解決できるか・ディレクトリか・
  通常ファイルか・まだ何も無いか）。存在しない資格情報ファイルと存在するものは同じく拒み、作業ディレクトリの外の
  存在しないパスと存在するパスも、文言と details まで同じに答える。存在は置き場所で確かめ、名指しの綴りから
  たどり直さない（置き場所が飛ばした部分を通ってしまう）。この修正の以前の版は、解決できないパスに別の分岐を
  与えたため、それぞれ答えの違う対を 1 つずつ残した。既知の例外は 1 つ: 作業ディレクトリの外に仕込まれた、
  資格情報ファイルへのハードリンクは資格情報として拒まれ、そこに無いパスは「外」になる —— pathguard の実体の
  比較は存在するファイルを要し、拒否文が資格情報を名指すよう封じ込めより先に走るため。
- ファイルの方針はシステムの場所を設計上含まない。システムのツリーの上にある作業ディレクトリは、root で動く
  ...
  `CheckBeneath` をファイルのあるディレクトリに両方向で当て、資格情報とサーバーの場所以外の拒否を残す（実際には
  システムのディレクトリとホームディレクトリそのもの）。これも当てる資格情報とサーバーの場所は、向きごとに判定するファイルの方針に任せる（重ねて
  当てると、`.env` という名前のディレクトリの規則が `.env` という名前の Python の venv を拒んだ）。
...
## 追補（2026-09-22, v0.5.1）: 置き場所は pathguard の `Where`

置き場所を pathguard の形（`Forms`）の末尾で決めていたが、形は重複を除くので、リンクの連鎖が既に出た綴りに戻ると
末尾は終点ではなく途中の段になる。pathguard v0.3.0 の `Where` が歩いた終点を返すので、置き場所はそれにした。
行き先が決まらないパス（終わらないリンクの連鎖）は渡されたパスのままで、床がそれを拒む。そのほかの限界は pathguard の
README の「限界」にまとめて記録し、受け入れた。
