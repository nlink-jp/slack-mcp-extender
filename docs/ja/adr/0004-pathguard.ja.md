...
- **パスが存在するかどうかで答えを変えない。** 両方向とも順序を 1 つにする: 解決 → 床 → 封じ込め → 存在に
  依存する検査（ディレクトリか・通常ファイルか・まだ何も無いか）。解決できないパスは床にかけ、次に存在する
  いちばん深い上位で置き場所を決め（外なら `outside_allowed_roots`）、そのあとで初めて `not_found` とする。
  存在しない資格情報ファイルと存在するものは同じく拒み、作業ディレクトリの外の存在しないパスと存在するパスも
  同じに答える。
- ファイルの方針はシステムの場所を設計上含まない。システムのツリーの上にある作業ディレクトリは、root で動く
  ...
  `CheckBeneath` をファイルのあるディレクトリに両方向で当て、システムのディレクトリとホームディレクトリそのものに
  よる拒否だけを残す。これも当てる資格情報とサーバーの場所は、向きごとに判定するファイルの方針に任せる（重ねて
