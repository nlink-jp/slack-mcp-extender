...
**App アイコン（任意）:** マニフェストにはアイコンを含められないため、
ワークスペース内で見分けやすくしたい場合は
[`docs/slack-app-icon.png`](../slack-app-icon.png)（1024×1024、SVG ソース
同梱）を **Basic Information → Display Information** からアップロード
してください。
...
`ext_file_upload`・`ext_file_upload_to_thread`・`ext_file_download` が使えるようになります。
